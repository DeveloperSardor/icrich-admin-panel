...
const LocalList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingId
        ? `${BACKEND_URL}/api/local-list/${editingId}`
        : `${BACKEND_URL}/api/local-list`;
      ...
      console.error("Error submitting local list:", error);
      toast.error(t("error"));
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(`${BACKEND_URL}/api/local-list/${id}`);
      if (response.data.success) {
      ...
      console.error("Error deleting local list:", error);
      toast.error(t("deleteError"));
    }
  };
  ...
  return (
    <div className="locallist-page">
      <Sidebar />
      <div className="locallist-content">
        <div className="locallist-header">
          <div>
            <h1>{t("localList")}</h1>
            <p className="locallist-subtitle">
              {t("totalLocalLists")}: <strong>{filteredDocs.length}</strong>{" "}
              {t("localListsCount")}
            </p>
          </div>
          <button onClick={() => openModal()} className="add-locallist-btn">
            <FiPlus size={20} />
            {t("addLocalList")}
          </button>
        </div>
        ...
        <div className="search-container">
          <div className="search-wrapper">
            ...
            <input
              type="text"
              placeholder={t("searchLocalLists")}
              value={searchQuery}
        ...
        {loading ? (
          ...
          <div className="empty-state">
            <FiList size={64} />
            <h3>{searchQuery ? t("noSearchResults") : t("noLocalLists")}</h3>
            <p>{searchQuery ? t("noSearchResults") : t("noLocalListsDesc")}</p>
          </div>
        ) : (
          <>
            <div className="locallist-grid">
              {currentDocs.map((doc) => (
                <div key={doc._id} className="locallist-card">
                  <div className="locallist-icon">
                    <FiList size={32} />
                  </div>

                  <div className="locallist-content-card">
                    <h3 className="locallist-title">
                      {currentLang === "en"
                        ? doc.title_en
                        : currentLang === "ru"
                        ? doc.title_ru
                  ...
                  <div className="locallist-actions">
                    <button
        ...
        <Modal isOpen={isModalOpen} onClose={closeModal} onSubmit={handleSubmit}>
          <div className="modal-form">
            ...
            <button type="submit" className="submit-btn">
              {editingId ? t("save") : t("addLocalList")}
            </button>
          </div>
        </Modal>
      </div>
